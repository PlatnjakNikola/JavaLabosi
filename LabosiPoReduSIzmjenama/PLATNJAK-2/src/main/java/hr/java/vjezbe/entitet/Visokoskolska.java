...
public interface Visokoskolska {
     ...
     */
    BigDecimal izracunajKonacnuOcjenuStudijaZaStudenta(Ispit[]ispits, int ocjenaPismenogDijela, int ocjenaObraneZavrsnogRada);
     ...
     */
    default BigDecimal odrediProsjekOcjenaNaIspitima(Ispit[] ispits) {
        Ispit[] ispiti = filtrirajPolozeneIspite(ispits);
        int suma = 0;
        for(Ispit ispit: ispiti){
            suma += ispit.getOcjena();
        }
        return new BigDecimal(suma / ispiti.length);
    }
